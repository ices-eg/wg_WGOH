...
for pp=1:size(rbc,1)
    if pp==1
        patch(tx_t,pp+[-0.5 0.5 0.5 -0.5 -0.5],[0 0 1 1 0],rbc(pp,:),'edgecolor','none')
        text(pp,yytxt,{'';'y';['< ' sprintf('% -3.1f',cbounds(pp))]}, ...
            'horizontalalignment','center','VerticalAlignment','bottom','interpreter','tex','FontSize',10);
        patch(tx_t,pp+[-0.5 0.5 0.5 -0.5 -0.5],[0 0 6 6 0],rbc(pp,:),'facecolor','none','edgecolor',lgrey)
...
set(tx_t,'color','none','XColor','none','ycolor','none','xlim',[0.5 size(rbc,1)+0.5],'ylim',[0 6])

% colourmap anomalies salt - bottom
cbounds =  -1*t_ext:0.5:t_ext+0.5;
yytxt = 1.1;
...
for pp=1:size(pgc,1)
    if pp==1
        patch(tx_s,pp+[-0.5 0.5 0.5 -0.5 -0.5],[0 0 1 1 0],pgc(pp,:),'edgecolor','none')
        text(pp,yytxt,{'';'y';['< ' sprintf('% -3.1f',cbounds(pp))]}, ...
            'horizontalalignment','center','VerticalAlignment','bottom','interpreter','tex','FontSize',10);
        patch(tx_s,pp+[-0.5 0.5 0.5 -0.5 -0.5],[0 0 6 6 0],pgc(pp,:),'facecolor','none','edgecolor',lgrey)
...
set(tx_s,'color','none','XColor','none','ycolor','none','xlim',[0.5 size(pgc,1)+0.5],'ylim',[0 6])

%adjust the size of the temperature and salinity plots and colour legends
set(tx_s,'position',[0.05 0.02 0.9 0.07])
set(tx_t,'position',[0.05 0.91 0.9 0.07])
set(ax_s,'Position',[0.05 0.135 0.9 0.35],'TickLength',[.002 .002]);
set(ax_t,'Position',[0.05 0.52 0.9 0.35],'TickLength',[.002 .002]);

xax_only = axes;
set(xax_only,'Position',[0.05 0.12 0.9 0.35],'color','none','YColor','none',...
    'Xlim',[ylower yhigher],'XTick',ylower:10:yhigher,'XMinorTick','off',...
    'TickLength',[.002 .002])

%save figure 
fname_fig = [timeseries_files{1},'_',datestr(now,'yyyymmdd'),...
        '_temp_sal_barplot_legend.png'];
